                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead>
                        <tr>
                            ...
                            <th class="w-3/6 px-1 py-3 pl-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 bg-blue-50" >{{ __($campo2) }} </th>
                            <th class="w-1/6 px-1 py-3 pl-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 bg-blue-50" >{{ __($campo3) }} </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 ">
                        @foreach ($valores as $valor)
                            <tr wire:loading.class.delay="opacity-50">
                                ...
                                <td class="w-3/6 px-1 text-xs leading-5 tracking-tighter text-left text-gray-600 whitespace-no-wrap">
                                    <input type="text" value="{{ $valor->nombre }}"
                                    wire:change="changeNombre({{ $valor }},$event.target.value)"
                                    class="w-full text-xs font-thin text-gray-500 border-0 rounded-md"/>
                                </td>
                                <td class="w-1/6 px-1 text-xs leading-5 tracking-tighter text-gray-600 whitespace-no-wrap">
                                    <input type="text" value="{{ $valor->aux }}"
